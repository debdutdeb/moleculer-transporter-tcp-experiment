...
class Instances extends Database {
    constructor() {
        super(
            process.env.DATABASE_FILE?.trim() ||
                require("path").join(__dirname, "instances.db")
        );
        if (process.env.CLEAN_TRY) {
            this.exec("drop table if exists instances;");
        }
        this.exec(
            "create table if not exists instances (id varchar(50) primary key, host varchar(50) not null, tcpPort integer not null);"
        );
    ...
    async addHost({ id, host, tcpPort }) {
        await this.add(
            "insert into instances(id, host, tcpPort) values(?, ?, ?)",
            [id, host, tcpPort]
        );
    }
